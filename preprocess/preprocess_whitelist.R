...
p0 <- ggplot(allencoded,aes(x=factor(GC),y=count))+  scale_y_log10()+geom_boxplot()
p1 <- ggplot(allencoded, aes(x = first_index, y = count, color=first_barcode))+geom_violin()+ scale_y_log10()+geom_jitter()
p2 <- ggplot(allencoded, aes(x = batch, y = count, color=batch))+geom_violin()+ scale_y_log10()+theme(axis.text.x = element_text(angle = 90, hjust = 1))

...
read_summary$raw_reads <- read_summary$raw_reads/16
read_summary$filtered_reads <- read_summary$filtered_reads/16
read_summary$aligned_reads <- read_summary$aligned_reads/16
read_summary$UMI_count <- read_summary$UMI_count/16
#
# fraction
...
ggplot(read_summary_ggplot,aes(x=factor(variable),y=value,color=plate))+geom_jitter()
